...
`,m={"/":()=>u(b()),"/profile":()=>u(p()),"/login":f,"*":x},a=()=>{const e=window.location.pathname.replace(n,"")||"/",t=document.querySelector("#root");if(e==="/login"&&r.state.loggedIn){window.history.pushState({},"",n+"/"),t.innerHTML=u(b());return}if(e==="/profile"&&!r.state.loggedIn){window.history.pushState({},"",n+"/login"),t.innerHTML=f();return}const l=m[e]||m["*"];t.innerHTML=l()};window.addEventListener("popstate",a);document.addEventListener("DOMContentLoaded",()=>{a(),document.body.addEventListener("click",e=>{if(e.target.matches('a[href^="/"]')){e.preventDefault();const t=e.target.getAttribute("href");window.history.pushState({},"",n+t),a()}})});
